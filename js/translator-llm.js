// translator-llm.js
// LLM HTTP helpers: model discovery, text cleaning, tiered summarization,
// translation quality validation, and the chunk translator with retry logic.
// Split out of translator.js so the HTTP/LLM concern is isolated from the
// translation pipeline and the preset/manifest concern. translator.js
// re-exports every symbol here so existing imports from ./translator.js
// (main.js, benchmark.js, ui.js) keep resolving unchanged.
...
function detectContextLeak(cleanedResult, currentContext) {
    for (let ctxLine of currentContext) {
        ...
        if (cleanCtx.length > 15) {
            if (cleanedResult.includes(cleanCtx)) {
                return { leaked: true, leakedLine: cleanCtx };
            }
    ...
    return { leaked: false, leakedLine: "" };
}
...
export async function translateChunkWithContext(host, model, chunkText, previousContext, presetType = 'jpEn', speakerName = '', tempAdjust = 0, sourceText = '') {
    ...
    while (attempts <= maxRetries) {
        ...
        const leakResult = detectContextLeak(cleanedResult, currentContext);
        const hasOldContext = leakResult.leaked;
        const leakedContextLine = leakResult.leakedLine;
